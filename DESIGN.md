...
1〜3 は「FBX マテリアル 1 つに .mat を 1 つ」決める順序。Unity で実際に表示されるのは Renderer の `m_Materials` で、externalObjects は FBX を置いたときの既定値にすぎないので、prefab に割り当てがあれば下記のスロット単位の分割で 1・2 の結果より優先する。 FBX がマテリアルを 1 つも持たない場合は 1〜3 の対象が無いので、prefab の割り当てだけからスロットを作る（下記「マテリアルを持たないモデル」）。
...
- 展開: Transform（RectTransform を含む）・GameObject・Renderer を読み、PrefabInstance は元のアセットを再帰的に差し込む（深さ 16、Transform 20 万個まで、循環は打ち切り）。差し込んだオブジェクトの fileID は prefab と同じく「PrefabInstance の fileID XOR 元の fileID」。シーンの fileID はこの規則に従わない（調査で確認）ので、外からの参照（子を付ける親、`m_TransformParent`）は stripped ドキュメントの `m_CorrespondingSourceObject` / `m_PrefabInstance` を対応表にして引く。展開した Node の key は fileID ではなく展開ごとの通し番号で、上書き・削除・stripped の参照は PrefabInstance ごとの「元のアセットでの fileID → 差し込んだ Node」の表で引く。fileID を連番に振り直したパッケージでは XOR の値が小さく偏り、別の PrefabInstance のものと重なって互いを上書きしていたため（Issue #107）。XOR の値はアセット全体の表にも載せ（重なれば先のものを残す）、外側から入れ子の中を指す上書きに使う。上書きは位置・回転・スケール、`m_IsActive`、`m_Name`、Renderer の `m_Enabled` とマテリアルを当て、`m_RemovedGameObjects` / `m_RemovedComponents` を除く。
- 配置は 2 種類。モデルの PrefabInstance は、そのルートの Transform（fileID は FBX によらず定数 -8679921383154817045）を配置のルートにする。中のオブジェクトへの上書きは、新しい形式では fileID を名前に結び付けられないので数えて警告に出す（Issue #31）。ただし、名前を引けないマテリアルの上書きがすべて同じ 1 つの target を指すときは、唯一の Renderer 宛て（`SOLE_RENDERER`）として配置に記録し、読み込んだモデルのメッシュのオブジェクトが 1 つならそれに当てる（下記「Renderer が 1 つのモデルの中への上書き」）。Unity 2018.2 以前の形式の .meta は `fileIDToRecycleName`（fileID → 名前）を持つので、それで名前を引けた上書きは当てる（下記「古い形式のモデルの中への上書き」）。モデルのメッシュを直接指す Renderer（FBX を展開した prefab や、FBX から切り離してシーンに置いた小物）は、Renderer ごとにモデルのルートを決め、同じルート・同じモデルの Renderer をまとめる（下記「Renderer をまとめるモデルのルート」）。
...
- LOD（Issue #104）: Unity の LOD は prefab・シーンの `LODGroup`（クラス ID 205）が持ち、`m_LODs` の段ごとに Renderer の fileID が並ぶ。段ごとの GameObject はどれも Renderer を持つので、そのままでは全段が重なって表示される。`parse_asset` で段を読み、`RendererInfo.lod_level`（0 = もっとも細かい LOD0）に付けて、読み込み側が LOD1 以降を非表示にする（削除はしない。段は `unity_lod` カスタムプロパティに残し、件数を警告に出す。ダイアログの「LODs」トグルで切れる）。LODGroup はシーンから prefab インスタンスの中の Renderer を指すことがあるので、段の解決は PrefabInstance の差し込みが終わった後に、stripped ドキュメントの別名を通して行う。1 つの LODGroup の中で同じ Renderer が複数の段にあればもっとも細かい段を採り、内側の prefab で付いた段は外側で上書きしない（内側で遠景用だったものは隠れたままにする）。`m_Enabled: 0` の LODGroup は Unity でも全段が描かれるので段を付けない。適用するのは Scenes と Prefabs 単位（Models 単位の表は prefab をまたいで統合したものなので対象にしない）。外側から LODGroup 自体を無効化・削除する上書きは反映しない。`_LOD1` のような命名規則からの推測はしない（LODGroup の無いモデルは今までどおり全部表示する）。
...
**Renderer が 1 つのモデルの中への上書き**（Issue #109）: 新しい形式のモデルの中の Renderer の fileID は階層パスのハッシュで、計算式が分かっていない（Issue #31）。それでも、モデルに Renderer が 1 つしか無ければ、上書きの対象はその Renderer に確定する。そこで `Expander._insert` は、表で名前を引けなかった `m_Materials.Array.data[N]` の上書きを target ごとに集め、target が 1 つならモデルの PrefabInstance のルートの `model_materials` に `SOLE_RENDERER` の名前で記録する（target が複数なら Renderer も複数なので、今までどおり数えて警告に出す）。Renderer の数は `.meta` の表が空だと分からないため、判定は読み込み側で行う。`build_model` がモデルを読んだ後に `prefab.resolve_sole_renderer` を呼び、メッシュのオブジェクトが 1 つならその行をオブジェクトの名前に付け替え（名前で引けた行があれば、空いたスロットだけを埋める）、複数なら当てずに件数を警告に出す。スロット数などから対象を推測することはしない（誤ると別の部位の色が変わるため）。上書きされていないスロットは、Unity と同じくモデル自身の割り当てのまま。`m_Materials.Array.size` の上書きは、どの Renderer か確定するまで長さを当てられないので数える。実例は Shibuya.unitypackage の `SM_MetroEntrance_02.prefab`（34 スロットのうち 17 を上書き）。
...
- 表の無いモデル（Issue #111）: `.meta` の表が空だと 1 メッシュの FBX か分からないので、展開ではルートの Transform への上書きを成分ごとに`node_transforms[SOLE_NODE]` へ記録し、`root_node` を `SOLE_NODE` にしておく（ルートを上書きしていなければ記録しない）。読み込み側は、読み込んだオブジェクトが子の無い 1 つだけなら畳まれた形とみなし、そのオブジェクトの名前で `apply_collapsed_root` を当てる。オブジェクトが複数なら Unity のルートは単位行列が基準なので、展開した行列のまま置く。実例は Shibuya.unitypackage の `SM_MetroEntrance_02`（ノードの回転は Unity で euler (270, 180, 0)。シーンが回転を (270, 2.47, 0) に上書きするので、Y の 180 度は Unity では消える）で、Unity 6000.6.0f1 の batchmode で求めたワールドのバウンディングボックスと突き合わせた。合成シーンでは、`SlabInstance` と同じ FBX を表の無い `.meta` で置いた `SlabBareInstance` で確かめる
- prefab で包んだ形（Issue #111）: 地下鉄の入口は、FBX をルートの位置 0・回転なしで置いた prefab を、シーンがさらに上書きしている。以前は FBX を直接置いた階層の上書きしか成分付きで記録しておらず、外側の上書きが読み込み側に渡っていなかった（表のあるモデルでも同じ）。そこで畳まれうるモデルのルートの Node に `collapsed_key`（表から求めたノード名か `SOLE_NODE`）を持たせ、`_apply_modification` が位置・回転・スケールを当てるときに、どの階層の上書きでも `model_transforms[collapsed_key]` に記録する。合成シーンの `SlabBareWrapped` / `SlabWrapped` で確かめる
...
**マテリアルを持たないモデル**（Issue #102）: FBX 自体がマテリアルを 1 つも持たないと、Blender ではマテリアルスロットが 0 個になり、差し替える相手が無い。Unity ではこのメッシュもサブメッシュ 1 つとして Renderer の `m_Materials[0]` で描かれるので、スロットが 0 個のメッシュには `m_Materials` の先頭をスロット 0 の割り当てとして返し（`core/mapping.py` の `_prefab_slots`）、読み込む側でスロットを作ってから入れる（`blender/importer.py` の `_split_slots_by_prefab`）。街並みの環境アセットの地面が、`.meta` の externalObjects も空でマテリアル無しのまま読み込まれていた。prefab の手掛かりも無いメッシュは、マテリアルを決めようが無いので警告を出して素のままにする。
...
- `tests/test_fbx_units.py`: FBX の UnitScaleFactor（バイナリの数値の型、FBX 6 の文字列属性の数、ASCII の FBX 7 / 6、壊れた値）。古い形式のモデルの中への上書きは、`tests/test_meta.py`（fileIDToRecycleName）、`tests/test_hierarchy.py`（名前付きの記録）、シーン用の合成パッケージの `Legacy.unity`（統合テスト）で確かめる。Renderer が 1 つのモデルの中への上書きは、`tests/test_prefab.py`（記録と付け替え）と、シーン用の合成パッケージの `Kiosk.prefab`（当てる）/ `Booth.prefab`（メッシュが 2 つなので当てずに警告する）で確かめる（統合テストの `warnings_contain` / `warnings_exclude`）。
