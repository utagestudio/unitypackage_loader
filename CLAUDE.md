...
- 紹介ページのローカル確認: `blender ... extension build` と `server-generate` を `site/` に出したあと `python3 tools/build_site.py site <base_url>`。
  ...
  `web/assets/placeholder-*.svg` は必要な構図を書き込んだダミー画像（使い道の 2・3 枚目）で、実際のスクリーンショットに差し替える前提。
  使い道の画像は全画面のスクリーンショットから 3:2 で切り出し、1200×800 の WebP にする（1 枚目は `use-edit.webp`）。
