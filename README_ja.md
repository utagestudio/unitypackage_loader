...
日本語 | [English](README.md) | [紹介ページ](https://utagestudio.github.io/unitypackage_loader/ja/) | [GitHub](https://github.com/utagestudio/unitypackage_loader)
...
- **Scenes**: シーン（`.unity`）に置かれたものを、Unity で設定された位置・回転・スケールのまま読み込みます。シーンごとに Collection ができ、
  ...
  prefab やシーンが使っていないモデルファイルの部品（使われていない LOD や余分なノードなど）は、Unity の Renderer が指すメッシュで照合して非表示にします。
  Unity の LODGroup がある場合は、もっとも細かい LOD0 だけを表示し、遠景用の段は読み込んだうえで非表示にします（ダイアログの **LODs** で切り替え。
  隠したオブジェクトはファイルに残り、段が `unity_lod` カスタムプロパティに入ります）。ライトとカメラも読み込みます（「Also Import」で切り替え）。
...
- **Prefabs**: prefab ごとに、その prefab のマテリアルの割り当てのまま読み込みます。パッケージの Collection の中に prefab ごとの Collection ができます。
  LODGroup のある prefab は、Scenes と同じく遠景用の LOD を非表示にします。
...
モデル（FBX）を直接置いたものへの上書きは、Unity 2018.2 以前の形式の `.meta`（名前の表がある）ならシーンの読み込みで当てます。
名前の表が無くても、モデルのメッシュが 1 つだけなら上書きの対象がそれに決まるので、マテリアルの上書きは当てます。それ以外は警告に出します。
